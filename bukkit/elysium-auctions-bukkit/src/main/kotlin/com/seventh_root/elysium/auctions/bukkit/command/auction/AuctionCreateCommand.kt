...
class AuctionCreateCommand(private val plugin: ElysiumAuctionsBukkit): CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (sender is Player) {
            if (sender.hasPermission("elysium.auctions.command.auction.create")) {
    ...
    private inner class AuctionCreatedPrompt: MessagePrompt() {
        ...
        override fun getNextPrompt(context: ConversationContext): Prompt {
            ...
            val character = characterProvider.getActiveCharacter(player)
            if (character != null) {
                val auction = ElysiumAuctionImpl(
                        plugin,
                        item = bukkitPlayer.inventory.itemInMainHand,
                        currency = context.getSessionData("currency") as ElysiumCurrency,
