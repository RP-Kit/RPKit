...
class QuickChatChannelCommand(private val plugin: RPKChatBukkit, private val chatChannel: RPKChatChannel) : RPKCommandExecutor {
    ...
    override fun onCommand(sender: RPKCommandSender, args: Array<out String>): CompletableFuture<out CommandResult> {
        if (!sender.hasPermission("rpkit.chat.command.chatchannel.${chatChannel.name.value}")) {
            sender.sendMessage(plugin.messages.noPermissionChatChannel.withParameters(
                chatChannel = chatChannel
            ))
            return completedFuture(NoPermissionFailure("rpkit.chat.command.chatchannel.${chatChannel.name.value}"))
        }
        ...
        if (args.isNotEmpty()) {
            chatChannel.sendMessage(profile, sender, args.joinToString(" "))
        } else {
            chatChannel.addSpeaker(sender)
            sender.sendMessage(plugin.messages.chatChannelValid.withParameters(
                chatChannel = chatChannel
            ))
        }
