...
class SenderCharacterCardFieldPart(
    ...
    override fun getText(context: DirectedPreFormatMessageContext): CompletableFuture<String> = CompletableFuture.supplyAsync {
        val characterService = Services[RPKCharacterService::class.java] ?: return@supplyAsync ""
        val senderMinecraftProfile = context.senderMinecraftProfile ?: return@supplyAsync ""
        val character = characterService.getActiveCharacter(senderMinecraftProfile).join() ?: return@supplyAsync ""
        val characterCardFieldService = Services[RPKCharacterCardFieldService::class.java] ?: return@supplyAsync ""
        val field = characterCardFieldService.characterCardFields.firstOrNull { it.name == field } ?: return@supplyAsync ""
        return@supplyAsync field.get(character).exceptionally { exception ->
            plugin.logger.log(SEVERE, "Failed to get character card field", exception)
            throw exception
        }.join()
    ...
    companion object {
        @JvmStatic
        fun deserialize(serialized: Map<String, Any>) = SenderCharacterCardFieldPart(
            Bukkit.getPluginManager().getPlugin("rpk-chat-bukkit") as RPKChatBukkit,
            serialized["field"] as String,
