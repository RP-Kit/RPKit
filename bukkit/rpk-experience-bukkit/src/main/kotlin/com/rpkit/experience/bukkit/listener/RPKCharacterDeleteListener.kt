...
class RPKCharacterDeleteListener(private val plugin: RPKExperienceBukkit) : Listener {

    @EventHandler
    fun onCharacterDelete(event: RPKBukkitCharacterDeleteEvent) {
        val characterId = event.character.id
        if (characterId != null) {
            plugin.database.getTable(RPKExperienceTable::class.java).delete(characterId)
        }
    }
}