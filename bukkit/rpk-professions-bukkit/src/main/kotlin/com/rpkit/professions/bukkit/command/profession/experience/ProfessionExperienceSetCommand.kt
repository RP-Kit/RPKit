...
class ProfessionExperienceSetCommand(val plugin: RPKProfessionsBukkit) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (!sender.hasPermission("rpkit.professions.command.profession.experience.set")) {
            sender.sendMessage(plugin.messages["no-permission-profession-experience-set"])
            return true
        }
        var argsOffset = 0
        val target = if (sender.hasPermission("rpkit.professions.command.profession.experience.set.other")) {
            when {
                args.size > 2 -> {
                    val player = plugin.server.getPlayer(args[0])
                    if (player == null) {
                        sender.sendMessage(plugin.messages["profession-experience-set-invalid-player-not-online"])
                        return true
                    } else {
                        argsOffset = 1
                        player
                ...
                sender is Player -> sender
                    ...
                    sender.sendMessage(plugin.messages["profession-experience-set-invalid-player-please-specify-from-console"])
                    return true
                }
            }
        } else {
