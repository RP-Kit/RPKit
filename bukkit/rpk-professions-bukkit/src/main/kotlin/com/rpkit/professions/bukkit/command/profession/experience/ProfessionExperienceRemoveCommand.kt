...
class ProfessionExperienceRemoveCommand(val plugin: RPKProfessionsBukkit) : CommandExecutor {

    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (!sender.hasPermission("rpkit.professions.command.profession.experience.remove")) {
            sender.sendMessage(plugin.messages["no-permission-profession-experience-remove"])
            return true
        }
        var argsOffset = 0
        val target = if (sender.hasPermission("rpkit.professions.command.profession.experience.remove.other")) {
            when {
                args.size > 2 -> {
                    val player = plugin.server.getPlayer(args[0])
                    if (player == null) {
                        sender.sendMessage(plugin.messages["profession-experience-remove-invalid-player-not-online"])
                        return true
                    } else {
                        argsOffset = 1
                        player
                ...
                sender is Player -> sender
                    ...
                    sender.sendMessage(plugin.messages["profession-experience-remove-invalid-player-please-specify-from-console"])
                    return true
                }
            }
        } else {
        ...
        if (args.size < 2) {
            sender.sendMessage(plugin.messages["profession-experience-remove-usage"])
            return true
        }
        val exp = try {
            args[argsOffset + 1].toInt()
        } catch (exception: NumberFormatException) {
            sender.sendMessage(plugin.messages["profession-experience-remove-invalid-exp-not-a-number"])
            return true
        }
