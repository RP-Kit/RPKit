...
class IssueCommand(private val plugin: RPKEssentialsBukkit) : RPKCommandExecutor {

    private val issueSubmitCommand = IssueSubmitCommand(plugin)

    override fun onCommand(sender: RPKCommandSender, args: Array<out String>): CompletableFuture<out CommandResult> {
        if (args.isEmpty()) {
            sender.sendMessage(plugin.messages.issueUsage)
            return completedFuture(IncorrectUsageFailure())
        }
        return when (args[0].lowercase()) {
            "submit", "create" -> issueSubmitCommand.onCommand(sender, args.drop(1).toTypedArray())
            else -> {
                sender.sendMessage(plugin.messages.issueUsage)
