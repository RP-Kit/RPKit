...
class RPKMinecraftProfileDeleteListener(private val plugin: RPKEssentialsBukkit) : Listener {
    ...
    fun onMinecraftProfileDelete(event: RPKBukkitMinecraftProfileDeleteEvent) {
        ...
        if (minecraftProfileId != null) {
            plugin.database.getTable(RPKLogMessagesEnabledTable::class.java).delete(minecraftProfileId)
            plugin.database.getTable(RPKPreviousLocationTable::class.java).delete(minecraftProfileId)
        }
    }
