...
class RPKCharacterDeleteListener(private val plugin: RPKEssentialsBukkit) : Listener {

    @EventHandler
    fun onCharacterDelete(event: RPKBukkitCharacterDeleteEvent) {
        val characterId = event.character.id
        if (characterId != null) {
            plugin.database.getTable(RPKTrackingDisabledTable::class.java).delete(characterId)
        }
    }
