...
class RPKWarpServiceImpl(override val plugin: RPKTravelBukkit) : RPKWarpService {
    override val warps: List<RPKWarp>
        get() = plugin.database.getTable(RPKWarpTable::class).getAll()

    override fun getWarp(name: String): RPKWarp? {
        return plugin.database.getTable(RPKWarpTable::class)[name]
    }
    ...
    override fun addWarp(warp: RPKWarp) {
        ...
        plugin.database.getTable(RPKWarpTable::class).insert(event.warp)
    }
    ...
    override fun updateWarp(warp: RPKWarp) {
        ...
        plugin.server.pluginManager.callEvent(event)
        if (event.isCancelled) return
        plugin.database.getTable(RPKWarpTable::class).update(event.warp)
    }
    ...
    override fun removeWarp(warp: RPKWarp) {
        ...
        plugin.server.pluginManager.callEvent(event)
        if (event.isCancelled) return
        plugin.database.getTable(RPKWarpTable::class).delete(event.warp)
    }
}