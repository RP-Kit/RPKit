...
class RPKCharacterDeleteListener(private val plugin: RPKPermissionsBukkit) : Listener {

    @EventHandler
    fun onCharacterDelete(event: RPKBukkitCharacterDeleteEvent) {
        val characterId = event.character.id
        if (characterId != null) {
            plugin.database.getTable(RPKCharacterGroupTable::class.java).delete(characterId)
        }
    }
