...
import com.rpkit.store.bukkit.storeitem.*
...
class PurchaseCommand(private val plugin: RPKStoresBukkit) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        minecraftProfileService.getMinecraftProfile(bukkitOfflinePlayer).thenAccept getMinecraftProfile@{ minecraftProfile ->
            if (minecraftProfile == null) {
                sender.sendMessage(
                    plugin.messages["no-minecraft-profile-other", mapOf(
            ...
            if (profile !is RPKProfile) {
                sender.sendMessage(
                    plugin.messages["no-profile-other", mapOf(
            ...
            storeItemService.getStoreItem(RPKStoreItemId(storeItemId)).thenAccept getStoreItem@{ storeItem ->
                if (storeItem == null) {
                ...
                purchaseService.addPurchase(purchase).thenRun {
                    sender.sendMessage(
                        plugin.messages["purchase-successful", mapOf(
                            ...
                            "store_item_cost" to String.format(
                                "%.02f",
                                storeItem.cost / 100.0
                            ) + plugin.config.getString("payments.currency")
                        )]
                    )
                }
            }
