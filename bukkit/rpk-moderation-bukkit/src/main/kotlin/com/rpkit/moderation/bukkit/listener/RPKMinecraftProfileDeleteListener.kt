...
class RPKMinecraftProfileDeleteListener(private val plugin: RPKModerationBukkit) : Listener {
    ...
    fun onMinecraftProfileDelete(event: RPKBukkitMinecraftProfileDeleteEvent) {
        ...
        if (minecraftProfileId != null) {
            plugin.database.getTable(RPKTicketTable::class.java).delete(minecraftProfileId)
            plugin.database.getTable(RPKVanishStateTable::class.java).delete(minecraftProfileId)
        }
    }
