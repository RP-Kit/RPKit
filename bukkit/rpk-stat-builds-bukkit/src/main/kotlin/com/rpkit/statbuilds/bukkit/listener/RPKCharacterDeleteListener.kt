...
class RPKCharacterDeleteListener(private val plugin: RPKStatBuildsBukkit) : Listener {

    @EventHandler
    fun onCharacterDelete(event: RPKBukkitCharacterDeleteEvent) {
        val characterId = event.character.id
        if (characterId != null) {
            plugin.database.getTable(RPKCharacterStatPointsTable::class.java).delete(characterId)
        }
    }
