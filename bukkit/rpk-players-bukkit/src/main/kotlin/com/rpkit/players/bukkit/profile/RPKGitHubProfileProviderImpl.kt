...
class RPKGitHubProfileServiceImpl(override val plugin: RPKPlayersBukkit) : RPKGitHubProfileService {
    ...
    override fun getGitHubProfile(id: Int): RPKGitHubProfile? {
        return plugin.database.getTable(RPKGitHubProfileTable::class)[id]
    }

    override fun getGitHubProfile(user: GHUser): RPKGitHubProfile? {
        return plugin.database.getTable(RPKGitHubProfileTable::class).get(user)
    }

    override fun getGitHubProfiles(profile: RPKProfile): List<RPKGitHubProfile> {
        return plugin.database.getTable(RPKGitHubProfileTable::class).get(profile)
    }
    ...
    override fun addGitHubProfile(profile: RPKGitHubProfile) {
        ...
        plugin.database.getTable(RPKGitHubProfileTable::class).insert(event.githubProfile)
    }
    ...
    override fun updateGitHubProfile(profile: RPKGitHubProfile) {
        ...
        plugin.server.pluginManager.callEvent(event)
        if (event.isCancelled) return
        plugin.database.getTable(RPKGitHubProfileTable::class).update(event.githubProfile)
    }
    ...
    override fun removeGitHubProfile(profile: RPKGitHubProfile) {
        ...
        plugin.server.pluginManager.callEvent(event)
        if (event.isCancelled) return
        plugin.database.getTable(RPKGitHubProfileTable::class).delete(event.githubProfile)
    }
