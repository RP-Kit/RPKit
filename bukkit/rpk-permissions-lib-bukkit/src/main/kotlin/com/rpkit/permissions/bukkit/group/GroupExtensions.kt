...
fun RPKProfile.addGroup(group: RPKGroup) {
    Services[RPKGroupService::class]?.addGroup(this, group)
}

fun RPKProfile.addGroup(group: RPKGroup, priority: Int) {
    Services[RPKGroupService::class]?.addGroup(this, group, priority)
}

fun RPKProfile.removeGroup(group: RPKGroup) {
    Services[RPKGroupService::class]?.removeGroup(this, group)
}

fun RPKProfile.hasPermission(node: String): Boolean {
    return Services[RPKPermissionsService::class]?.hasPermission(this, node)
            ?: Bukkit.getPluginManager().getPermission(node)?.default?.getValue(false)
            ?: false
}

val RPKProfile.groups
    get() = Services[RPKGroupService::class]?.getGroups(this) ?: emptyList()
...
fun RPKMinecraftProfile.assignPermissions() {
    Services[RPKPermissionsService::class]?.assignPermissions(this)
}

fun RPKMinecraftProfile.unassignPermissions() {
    Services[RPKPermissionsService::class]?.unassignPermissions(this)
}

fun RPKGroup.hasPermission(node: String, default: Boolean): Boolean {
    return Services[RPKPermissionsService::class]?.hasPermission(this, node, default)
            ?: Bukkit.getPluginManager().getPermission(node)?.default?.getValue(false)
            ?: false
}
