...
public class ChatChannelSetIRCWhitelistCommand implements CommandExecutor {

    private final ElysiumChatBukkit plugin;
    private final ConversationFactory conversationFactory;

    public ChatChannelSetIRCWhitelistCommand(ElysiumChatBukkit plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Conversable) {
            if (sender.hasPermission("elysium.chat.command.chatchannel.set.ircwhitelist")) {
                ...
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("messages.no-permission-chatchannel-set-ircwhitelist")));
            }
        } else {
    ...
    private class ChatChannelIRCWhitelistSetPrompt extends MessagePrompt {
        ...
        public String getPromptText(ConversationContext context) {
            ...
            chatChannel.setIRCWhitelist((boolean) context.getSessionData("irc_whitelist"));
