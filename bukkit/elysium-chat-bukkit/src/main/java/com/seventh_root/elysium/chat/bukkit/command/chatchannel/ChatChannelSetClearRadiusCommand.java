...
public class ChatChannelSetClearRadiusCommand implements CommandExecutor {
    private final ElysiumChatBukkit plugin;
    private final ConversationFactory conversationFactory;

    public ChatChannelSetClearRadiusCommand(ElysiumChatBukkit plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Conversable) {
            if (sender.hasPermission("elysium.chat.command.chatchannel.set.clearradius")) {
                ...
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("messages.no-permission-chatchannel-set-clearradius")));
            }
        } else {
    ...
    private class ChatChannelClearRadiusSetPrompt extends MessagePrompt {
        ...
        public String getPromptText(ConversationContext context) {
            ...
            chatChannel.setClearRadius((int) context.getSessionData("clear_radius"));
