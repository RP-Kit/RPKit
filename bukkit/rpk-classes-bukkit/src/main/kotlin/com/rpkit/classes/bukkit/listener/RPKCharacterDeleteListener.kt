...
class RPKCharacterDeleteListener(private val plugin: RPKClassesBukkit) : Listener {

    @EventHandler
    fun onCharacterDelete(event: RPKBukkitCharacterDeleteEvent) {
        val characterId = event.character.id
        if (characterId != null) {
            plugin.database.getTable(RPKCharacterClassTable::class.java).delete(characterId)
            plugin.database.getTable(RPKClassExperienceTable::class.java).delete(characterId)
        }
    }
