...
class RPKCharacterDeleteListener(private val plugin: RPKUnconsciousnessBukkit) : Listener {

    @EventHandler
    fun onCharacterDelete(event: RPKBukkitCharacterDeleteEvent) {
        val characterId = event.character.id
        if (characterId != null) {
            plugin.database.getTable(RPKUnconsciousStateTable::class.java).delete(characterId)
        }
    }
