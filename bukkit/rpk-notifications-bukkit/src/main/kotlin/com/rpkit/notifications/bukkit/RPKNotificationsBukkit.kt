...
class RPKNotificationsBukkit : RPKBukkitPlugin() {
        ...
        database = Database(
            ...
            DatabaseMigrationProperties(
                when (databaseSqlDialect) {
                    "MYSQL" -> "com/rpkit/notifications/migrations/mysql"
                    "SQLITE" -> "com/rpkit/notifications/migrations/sqlite"
                    else -> throw UnsupportedDatabaseDialectException("Unsupported database dialect $databaseSqlDialect")
                },
                "flyway_schema_history_notifications"
            ),
            classLoader
        )
    ...
    private fun registerCommands() {
        getCommand("notification")?.setExecutor(NotificationCommand(this).toBukkit())
    }
