...
class NotificationDismissCommand(private val plugin: RPKNotificationsBukkit) : RPKCommandExecutor {
    ...
    class InvalidNotificationIdNotANumberFailure : CommandFailure()
    class InvalidNotificationRecipientFailure : CommandFailure()
    class InvalidNotificationFailure : CommandFailure()

    override fun onCommand(sender: RPKCommandSender, args: Array<out String>): CompletableFuture<CommandResult> {
        if (!sender.hasPermission("rpkit.notifications.command.notification.dismiss")) {
            sender.sendMessage(plugin.messages.noPermissionNotificationDismiss)
            return completedFuture(NoPermissionFailure("rpkit.notifications.command.notification.dismiss"))
        }
        if (args.isEmpty()) {
        ...
        return notificationService.getNotification(notificationId).thenApplyAsync { notification ->
            if (notification == null) {
                sender.sendMessage(plugin.messages.notificationDismissInvalidNotification)
                return@thenApplyAsync InvalidNotificationFailure()
            }
