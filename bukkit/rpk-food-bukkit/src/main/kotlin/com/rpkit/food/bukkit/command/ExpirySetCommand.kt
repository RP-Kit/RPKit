...
class ExpirySetCommand(private val plugin: RPKFoodBukkit) : RPKCommandExecutor {
    ...
    override fun onCommand(sender: RPKCommandSender, args: Array<out String>): CompletableFuture<out CommandResult> {
        ...
        if (!sender.hasPermission("rpkit.food.command.expiry.set")) {
            sender.sendMessage(plugin.messages.noPermissionExpirySet)
            return completedFuture(NoPermissionFailure("rpkit.food.command.expiry.view"))
        }
        if (args.size < 1) {
            sender.sendMessage(plugin.messages.expirySetUsage)
        ...
        val expiryHours = args[0].toLongOrNull()
        if (expiryHours == null) {
            sender.sendMessage(plugin.messages.expirySetInvalidHours)
            return completedFuture(InvalidHoursFailure())
        }
        val itemInHand = bukkitPlayer.inventory.itemInMainHand
        if (!itemInHand.type.isEdible) {
            sender.sendMessage(plugin.messages.expirySetInvalidInedibleItem)
            return completedFuture(InedibleFailure())
        }
        expiryService.setExpiry(itemInHand, Duration.ofHours(expiryHours))
        bukkitPlayer.inventory.setItemInMainHand(itemInHand)
        sender.sendMessage(plugin.messages.expirySetValid)
        return completedFuture(CommandSuccess)
    }

    class InvalidHoursFailure : CommandFailure()
    class InedibleFailure : CommandFailure()
